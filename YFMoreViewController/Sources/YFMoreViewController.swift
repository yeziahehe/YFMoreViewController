...
public enum YFMoreItemType {
    case important  // 将item放在第一行显示
    case normal     // 将item放在第二行显示
}

public class YFMoreViewController: UIViewController {
    ...
    deinit {
        print("deinit YFMoreViewController")
    }
...
extension YFMoreViewController {
    ...
    @objc fileprivate func handleButtonClickEvent(_ sender: Any) {
        let item: YFMoreItemView = sender as! YFMoreItemView
        if item.superview == importantItemsScrollView {
            delegate?.moreview(self, didSelectItemAt: importantItems.index(of: item)!, type: .important)
            delegate?.moreView(self, didSelectItemAt: item.itemTag, type: .important)
        } else if item.superview == normalItemsScrollView {
            delegate?.moreview(self, didSelectItemAt: normalItems.index(of: item)!, type: .normal)
            delegate?.moreView(self, didSelectItemAt: item.itemTag, type: .normal)
        }
        hideToButtom()
    }
    ...
    public func addItems(title: String, selectedTitle: String, image: UIImage, selectedImage: UIImage, type: YFMoreItemType, tag: String) {
        let itemView: YFMoreItemView = createItem(title: title, selectedTitle: selectedTitle, image: image, selectedImage: selectedImage, type: type, tag: tag)
    ...
    public func addItems(title: String, image: UIImage, type: YFMoreItemType, tag: String) {
        addItems(title: title, selectedTitle: title, image: image, selectedImage: image, type: type, tag: tag)
    }
    
    fileprivate func createItem(title: String, selectedTitle: String, image: UIImage, selectedImage: UIImage, type: YFMoreItemType, tag: String) -> YFMoreItemView {
        let itemView: YFMoreItemView = YFMoreItemView.init()
    ...
    fileprivate func insertItem(_ itemView: YFMoreItemView, at index: Int) {
        if itemView.itemType == .important {
    ...
    public func itemAtTag(_ tag: String) -> YFMoreItemView{
    ...
    private func viewWithTag(_ array: [YFMoreItemView], tag: String) -> YFMoreItemView? {
        for item in array {
            if item.itemTag == tag {
                return item
            }
